...
CREATE DATABASE ConsultasEjercicio2;
SHOW DATABASES;
use ConsultasEjercicio2;
...
SELECT NombreCompleto, Num_D, Nombre_D FROM Empleado, Pertenece, Departamento
WHERE Empleado.CI = Pertenece.CI AND Departamento.Num_D = Pertenece.Numero AND Numero = "7";

SELECT NombreCompleto FROM Supervisor, Empleado
WHERE Supervisor.CISupervisor = Empleado.CI
AND CIEmpleado = CI
